...
-- 2025-08-27,IOdawa,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_IOdawa')
	 Begin 
	  Alter Database [Assignment07DB_IOdawa] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_IOdawa;
	 End
	Create Database Assignment07DB_IOdawa;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_IOdawa;
...
SELECT 
    P.ProductName,
   -- formatting the unit price in US currency
...
go
SELECT 
    P.ProductName, InventoryDate = DATENAME(MM, I.InventoryDate) + ' ,'  + DateName(YY, I.InventoryDate),
...
SELECT  TOP 1000000
    C.CategoryName,
    InventoryDate = DATENAME(MM, I.InventoryDate) + ' ,'  + DateName(YY, I.InventoryDate),
    [TotalInventoryCountByCategory] = SUM(I.[Count])
...
ORDER BY CategoryName, MONTH([InventoryDate]), TotalInventoryCountByCategory
GO
...
SELECT  TOP 1000000
	...
	-- Any Inventory count that is null in the table will be set to zero
...
-- ORDER BY is not allowed inside a view unless using FOR XML or OFFSET-FETCH
GO
